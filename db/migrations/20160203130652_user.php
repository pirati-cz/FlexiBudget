        ...
        switch ($this->adapter->getAdapterType()) {
            case 'postges':
                $table->addColumn('icon', 'text',
                    ['default' => $anybody])->update();
                break;
            case 'mysql':
                $table->addColumn('icon', 'text')->update();
                $this->execute('CREATE TRIGGER new_insert
BEFORE INSERT ON `user`
FOR EACH ROW
SET NEW.`icon` = CASE WHEN NEW.icon IS NULL THEN \''.$anybody.'\' ELSE NEW.icon END
;');
                break;
            default:
                $table->addColumn('icon', 'text')->update();
