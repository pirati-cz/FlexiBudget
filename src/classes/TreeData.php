...
/**
 * Description of TreeData
 *
 * @author vitex
 */
class TreeData extends \Ease\Brick
{
    /**
     * Yes Use Database
     * @var boolean 
    ...
    public function get_children($id, $recursive = false)
    {
        ...
        return $this->dblink->queryToArray($sql);
    }
    ...
    public function mk($parent, $position = 0, $data = [])
    {
        $parent = (int) $parent;
        if ($parent == 0) {
            throw new \Ease\Exception('Parent is 0');
        }
        ...
        if (!$parent['children']) {
            ...
            $ref_rgt = $parent['children'][(int) $position][$this->options['structure']["left"]]
                + 1;
        }
        $sql[] = "
			UPDATE ".$this->options['structure_table']."
    ...
    public function mv($id, $parent, $position = 0)
    {
        ...
        $parent = $this->get_node($parent,
            ['with_children' => true, 'with_path' => true]);
        $id     = $this->get_node($id,
            ['with_children' => true, 'deep_children' => true, 'with_path' => true]);
        if (!$parent['children']) {
            $position = 0;
        }
        if ($id[$this->options['structure']['parent_id']] == $parent[$this->options['structure']['id']]
            && $position > $id[$this->options['structure']['position']]) {
            $position ++;
        }
        if ($parent['children'] && $position >= count($parent['children'])) {
            $position = count($parent['children']);
        }
        if ($id[$this->options['structure']['left']] < $parent[$this->options['structure']['left']]
            && $id[$this->options['structure']['right']] > $parent[$this->options['structure']['right']]) {
        ...
        $width = (int) $id[$this->options['structure']["right"]] - (int) $id[$this->options['structure']["left"]]
            + 1;
    ...
    public function cp($id, $parent, $position = 0)
    {
        ...
        $fields                                       = array_combine($this->options['structure'],
        ...
        $sql[]                                        = "
			INSERT INTO ".$this->options['structure_table']." ( ".implode(',',
                array_keys($fields))." )
			SELECT  ".implode(',', array_values($fields))." FROM ".$this->options['structure_table']." WHERE ".$this->options['structure']["id"]." IN (".implode(",",
                $tmp).")
        ...
        $new_nodes = $this->db->get("
			SELECT  * FROM ".$this->options['structure_table']."
			WHERE ".$this->options['structure']["left"]." > ".$ref_lft." AND ".$this->options['structure']["right"]." < ".($ref_lft
            + $width - 1)." AND ".$this->options['structure']["id"]." != ".$iid."
			ORDER BY ".$this->options['structure']["left"]."
		");
        $parents   = [];
        ...
        foreach ($sql as $k => $v) {
            try {
                ...
                $this->rm($iid);
                $this->reconstruct();
                throw new \Ease\Exception('Error copying');
            }
        }
        return $iid;
    }
    ...
    public function rm($id)
    {
        ...
        $data = $this->get_node($id,
            ['with_children' => true, 'deep_children' => true]);
        ...
        foreach ($sql as $v) {
            try {
                ...
                $this->reconstruct();
                throw new \Ease\Exception('Could not remove');
            }
        }
        return true;
    }
    ...
    public function rn($id, $data)
    {
        if (!(int) $this->dblink->queryToValue('SELECT  1 AS res FROM '.$this->options['structure_table'].' WHERE '.$this->options['structure']['id'].' = '.(int) $id)) {
            throw new \Ease\Exception('Could not rename non-existing node');
        }
        $tmp = [];
        ...
        if (count($tmp)) {
            ...
            $par                                   = array_merge(array_values($tmp),
                array_values($tmp));
            try {
                $this->dblink->exeQuery($this->assignParams($sql, $par));
            } catch (Exception $e) {
                throw new \Ease\Exception('Could not rename');
            }
        }
        return true;
    }
    ...
    public function analyze($get_errors = false)
    {
        ...
        if ((int) $this->dblink->queryToValue("
			...
			WHERE
				...
				(SELECT  COUNT(".$this->options['structure']['id'].") FROM ".$this->options['structure_table']." WHERE ".$this->options['structure']["id"]." = s.".$this->options['structure']["parent_id"].") = 0")
            > 0
        ) {
    ...
    public function reconstruct($analyze = true)
    {
        ...
        while ($counter <= $maxcounter) {
            ...
            if ($this->db->nextr()) {
                ...
                $q = "INSERT INTO temp_stack (stack_top, ".$this->options['structure']["id"].", ".$this->options['structure']["left"].", ".$this->options['structure']["right"].", ".$this->options['structure']["level"].", ".$this->options['structure']["parent_id"].", ".$this->options['structure']["position"].") VALUES(".($currenttop
                    + 1).", ".$tmp.", ".$counter.", NULL, ".$currenttop.", ".$this->db->f("pid").", ".$this->db->f("lid").")";
                if (!$this->db->query($q)) {
                    return false;
                }
    ...
    public function dump()
    {
        ...
        foreach ($nodes as $node) {
            echo str_repeat(" ",
                (int) $node[$this->options['structure']["level"]] * 2);
        ...
        try {
            ...
            switch ($_GET['operation']) {
                case 'analyze':
                    ...
                    break;
                case 'get_node':
                    $node = isset($_GET['id']) && $_GET['id'] !== '#' ? (int) $_GET['id']
                            : 0;
                    $temp = $this->get_children($node);
                    $rslt = [];
                    if (count($temp)) {
                        foreach ($temp as $v) {
                            $rslt[] = ['id' => $v['id'], 'text' => $v['nm'], 'icon' => $v['icon'],
                                'a_attr' => ['href' => $v['url']], 'children' => ($v['rgt']
                                - $v['lft'] > 1)];
                        }
                    }
                    break;
                case "get_content":
                    ...
                    if (count($node) > 1) {
                        ...
                        $temp = $this->get_node((int) $node[0],
                            ['with_path' => true]);
                        $rslt = ['content' => 'Selected: /'.implode('/',
                ...
                case 'create_node':
                    $node = isset($_GET['id']) && $_GET['id'] !== '#' ? (int) $_GET['id']
                            : 0;
                    $temp = $this->mk($node,
                        isset($_GET['position']) ? (int) $_GET['position'] : 0,
                ...
                case 'move_node':
                    $node = isset($_GET['id']) && $_GET['id'] !== '#' ? (int) $_GET['id']
                            : 0;
                    $parn = isset($_GET['parent']) && $_GET['parent'] !== '#' ? (int) $_GET['parent']
                            : 0;
                    $rslt = $this->mv($node, $parn,
                        isset($_GET['position']) ? (int) $_GET['position'] : 0);
                    break;
                case 'copy_node':
                    $node = isset($_GET['id']) && $_GET['id'] !== '#' ? (int) $_GET['id']
                    ...
                    $parn = isset($_GET['parent']) && $_GET['parent'] !== '#' ? (int) $_GET['parent']
                            : 0;
                    $rslt = $this->cp($node, $parn,
                        isset($_GET['position']) ? (int) $_GET['position'] : 0);
                    break;
                default:
                    throw new \Ease\Exception('Unsupported operation: '.$_GET['operation']);
                    break;
            }
